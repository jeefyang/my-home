            ...
            <div class="dc-panel dc-panel-output">
                ...
                <div class="dc-output mag-output" ref="outputRef">
                    <n-flex class="dc-output-actions" size="small">
                        ...
                        ></n-button>
                        <n-button v-if="outputLink" quaternary size="tiny" type="info" @click="openUrl(outputLink)" title="打开"
                            ><template #icon><n-icon :component="Link" size="14" /></template
                        ></n-button>
                    </n-flex>
...
const examples = ["C4B7E5D5A5E5A5E5A5E5A5E5A5E5A5E5A5E5A5", "magnet:?xt=urn:btih:C4B7E5D5A5E5A5E5A5E5A5E5A5E5A5E5A5E5A5", "赵钱孙李周吴郑王冯陈褚卫蒋沈韩杨"];
...
const copyOutput = async () => {
    Utils.copyStr(msg,outputText.value).then(() => msg.success("已复制"));
};
...
const convert = () => {
    ...
    if (!raw) {
        msg.warning("请输入磁力链接、Info Hash 或百家姓暗号");
        return;
    }
    ...
    if (clean.startsWith("magnet:?")) {
        ...
        if (hasChinese) {
            ...
            for (const ch of clean) {
                const h = surnameMap81[ch];
                if (h !== undefined) decoded += h;
                else unknown += ch;
            }
