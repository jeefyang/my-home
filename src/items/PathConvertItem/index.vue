        ...
        <div class="ps-options">
            <n-flex justify="space-between" align="center">
                ...
                <n-flex align="center" size="small">
                    <n-button size="tiny" quaternary @click="pasteInput"
                        ><template #icon><n-icon :component="Clipboard" size="14" /></template
                    ></n-button>
                    <n-button size="tiny" quaternary type="error" @click="clearInput"
...
const copyOutput = async () => {
    Utils.copyStr(msg, outputText.value, { successMsg: "已复制" });
};
...
.ps-output:hover .ps-output-actions,
.ps-output-actions:focus-within {
    opacity: 1;
}
@media (hover: none) {
    .ps-output-actions {
...
.ps-output::-webkit-scrollbar-thumb:hover {
    background: #585b70;
}
.ps-result {
